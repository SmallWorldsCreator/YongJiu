...
#region "Math"
	...
	public static float LoopIn(float p_num, float p_start, float p_end, float p_snapInt = 0){
		float _len = p_end - p_start;
		return LoopLen (p_num, p_start, _len, p_snapInt);
	}
	public static float LoopLen(float p_num, float p_start, float p_len, float p_snapInt = 0){
		p_num -= p_start;
		if (p_snapInt == 0) {
		} else if ((p_num - Mathf.Floor (p_num)) < p_snapInt) {
			p_num = Mathf.Floor (p_num);
		} else if ((Mathf.Ceil (p_num) - p_num) < p_snapInt) {
			p_num = Mathf.Ceil (p_num);
		}

		p_num = (p_num % p_len);
		p_num = ((p_num < 0) ? (p_num+p_len) : p_num);

		return p_num + p_start;
	}
